...
from models import init_db, UserProfile, AgentSession, JobApplication, AgentStatus, ApplicationStatus
...
async def start_agent(req: AgentStartRequest):
    ...
    asyncio.create_task(
        _run_agent_bg(pid, session_id, req.min_match_score, req.max_applications)
    )

    return {"session_id": session_id, "status": "started"}


async def _run_agent_bg(profile_id: int, session_id: int,
    ...
    async with ASession(engine) as db:
        res = await db.execute(select(UserProfile).where(UserProfile.id == profile_id))
