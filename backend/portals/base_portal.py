...
class BasePortalAgent(ABC):
    ...
    async def start(self):
        ...
        self.browser = await self._playwright.chromium.launch(
            ...
            args=["--no-sandbox", "--disable-blink-features=AutomationControlled",
                  "--disable-dev-shm-usage", "--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"]
        )
    ...
    async def safe_click(self, selector: str, timeout: int = 5000):
        ...
        try:
            ...
            raise Exception(f"safe_click failed for {selector}: {e}")
